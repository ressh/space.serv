...
class Player extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('summ_buy, summ_exit, summ_limit, activate_email, activate_phone', 'numerical', 'integerOnly'=>true),
    ...
    public function setReferalBonuses( $summ_pay, $statistic )
    {
        ...
        if( $this->role == 'partner' )
        {
            ...
            foreach (Yii::app()->params['referals']['counts'] as $key => $count) {
                ...
                if ($statistic->referals >= $count && $this->referals < Yii::app()->params['referals']['counts'][$key + 1]) {

                    // Определить сумму поступления пригласившего на счет покупок
                    $summ_buy_for_parent_referal = $summ_pay * Yii::app()->params['referals']['summ'][$key] / 100;
                    $this->setSummBuyPlus($summ_buy_for_parent_referal);

                    // Определить сумму поступления пригласившего на лимитированный счет
                    $summ_limit_parent_referal = $summ_pay * Yii::app()->params['referals']['summ_limit'][$key] / 100;

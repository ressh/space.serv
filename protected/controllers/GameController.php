...
class GameController extends Controller
{
    ...
    public function accessRules()
    {
        return array(
            ...
            array('allow',  // allow all users to perform 'list' and 'show' actions
                'actions' => array('Galactic', 'News', 'Support', 'captcha', 'lookRandomFight'),
                'users' => array('*'),
            ),
    ...
    public function  actionMissions()
    {
        ...
        $this->render('missions', array('player' => $player, 'ship' => $ship));
    }
    ...
    public function actionGalactic()
    {
        ...
        $pirats = Ship::model()->findAll(array('condition' => "type_ship=:type", 'params' => array(":type" => 'pirat'), 'order' => 'summ DESC', 'limit' => '10'));
        $killers = Ship::model()->findAll(array('condition' => "type_ship=:type", 'params' => array(":type" => 'killer'), 'order' => 'summ DESC', 'limit' => '10'));
        $sellers = Ship::model()->findAll(array('condition' => "type_ship=:type", 'params' => array(":type" => 'seller'), 'order' => 'summ DESC', 'limit' => '10'));
        $workers = Ship::model()->findAll(array('condition' => "type_ship=:type", 'params' => array(":type" => 'worker'), 'order' => 'summ DESC', 'limit' => '10'));
        ...
        $this->render('galactic', array('types' => $ships));
    }
    ...
    public function actionArena()
    {
        ...
        if ($current_gamaes != null) {
            foreach ($current_gamaes as $key => $game) {
                $warior_ship = Ship::model()->findAllByAttributes(array('id' => $game->creator_id));
            ...
            if ($room != null) {
                ...
                if ($room->is_complete == 0) {
                    if ($room->getCountPlayers() < $room->quantity) {
                        ...
                        if ($room->type_money == 0) {
                            if (!$player->setSummBuyPlus(-$room->summ)) {
                                $room->addError('summ', 'У вас не хватает денег, чтобы присоединиться');
                                $room->delete();
                                $this->render('add_join_strategy', array('ship' => $ship, 'player' => $player, 'room' => $room, 'join' => $join));
                                return;
                            }
                        ...
                        $room->addError('summ', 'Игра уже сыграна, вы не успели!');
                        $this->render('add_join_strategy', array('ship' => $ship, 'player' => $player, 'room' => $room, 'join' => $join));
                        return;
                    }
                } else {
                ...
                $room->addError('summ', 'Возможно вы неправильно выбрали игру!');
                $this->render('add_join_strategy', array('ship' => $ship, 'player' => $player, 'room' => $room, 'join' => $join));
                return;
    ...
    public function actionGetMoneyWin()
    {

        $player = Player::model()->findByAttributes(array('email' => Yii::app()->user->id));
        $game = ArenaRoom::model()->findByPk($_GET['id']);
    ...
    public function actionCloseFight()
    {
    ...
    public function  actiongetMoneyNothin()
    {
        $player = Player::model()->findByAttributes(array('email' => Yii::app()->user->id));
        $game = ArenaRoom::model()->findByPk($_GET['id']);

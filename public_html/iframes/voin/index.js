...
function addDialog() {
    setDialog($('.player'), 'Вот и встретились! Я обещал Гильдии, что найду тебя!', 1, 7000);
    ...
    createjs.Tween.get(exportRoot.ship)
        .to({x: exportRoot.ship.x + 180}, 1500, createjs.Ease.cubicOut)
        .call(handleComplete);
    function handleComplete() {
...
function destroyEnemy() {
    ...
    setDialog($('.player'), 'Это было легко...А награда неплохая)', 5000, 6000);

    setTimeout(function () {
        createjs.Tween.get(exportRoot.ship)
            .to({x: exportRoot.ship.x + 780}, 4500, createjs.Ease.cubicIn)

        exportRoot.ship.fire.alpha = 1;
